...
- `dse`, `dse_ms`, `dse_to_ts` и `dse_to_ts_ms` — краткие convenience-имена для
  конверсий числа суток с эпохи.
...
Time Shield использует семантический стиль номеров версий и разделяет API на
три уровня стабильности:

- **Canonical API** — основные документированные типы и функции. Ломающие
  изменения требуют отдельного решения по совместимости и версии.
- **Convenience API** — поддерживаемые альтернативные имена и короткие алиасы
  для частых операций. Наличие канонического эквивалента само по себе не
  делает алиас устаревшим.
- **Legacy compatibility API** — исторически переименованные или вытесненные
  имена с установленной заменой. Такие имена могут быть отключены по умолчанию,
  если доступен явный opt-in, например
  `TIME_SHIELD_ENABLE_LEGACY_ALIASES`.

Patch-релизы могут содержать исправления, внутренний рефакторинг, изменения
документации, небольшие дополнительные возможности и очистку compatibility API.
Minor-релизы предназначены для более крупных возможностей и развития API.
Major-релизы зарезервированы для намеренных существенных изменений канонического API.

Символ не считается legacy только потому, что он короткий, длинный,
избыточный или стилистически отличается. Статус legacy подтверждается историей
репозитория, документацией, явным переименованием или установленной заменой.
