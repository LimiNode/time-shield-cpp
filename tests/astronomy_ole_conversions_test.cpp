...
int main() {
    ...
    TIME_SHIELD_TEST_CHECK(std::fabs(oa_epoch - static_cast<oadate_t>(OLE_EPOCH)) < epsilon);

    const oadate_t oa_half_day = ts_to_oadate(static_cast<ts_t>(SEC_PER_DAY / 2));
    TIME_SHIELD_TEST_CHECK(std::fabs(oa_half_day - (static_cast<oadate_t>(OLE_EPOCH) + 0.5)) < epsilon);

    const oadate_t oa_zero = to_oadate(1899, 12, 30, 0, 0, 0, 0);
    TIME_SHIELD_TEST_CHECK(std::fabs(oa_zero) < epsilon);
    TIME_SHIELD_TEST_CHECK(std::fabs(to_oadate(1899, 12, 29, 6, 0, 0, 0) - static_cast<oadate_t>(-1.25)) < epsilon);
    TIME_SHIELD_TEST_CHECK(std::fabs(to_oadate(1899, 12, 30, 6, 0, 0, 0) - static_cast<oadate_t>(0.25)) < epsilon);

    const ts_t round_trip_ts = oadate_to_ts(oa_half_day);
    TIME_SHIELD_TEST_CHECK(round_trip_ts == static_cast<ts_t>(SEC_PER_DAY / 2));
    TIME_SHIELD_TEST_CHECK(oadate_to_ts(static_cast<oadate_t>(-1.25)) == pre_base_6h);
    TIME_SHIELD_TEST_CHECK(oadate_to_ts(static_cast<oadate_t>(-1.75)) == pre_base_18h);
    TIME_SHIELD_TEST_CHECK(oadate_to_ts(static_cast<oadate_t>(-0.25)) == base_6h);
    TIME_SHIELD_TEST_CHECK(oadate_to_ts(static_cast<oadate_t>(-0.25)) == oadate_to_ts(static_cast<oadate_t>(0.25)));
    TIME_SHIELD_TEST_CHECK(std::fabs(oadate_to_fts(static_cast<oadate_t>(-1.25)) - static_cast<fts_t>(pre_base_6h)) < epsilon);

    const ts_ms_t day_ms = static_cast<ts_ms_t>(MS_PER_DAY);
    const oadate_t oa_ms = ts_ms_to_oadate(day_ms);
    TIME_SHIELD_TEST_CHECK(std::llabs(oadate_to_ts_ms(oa_ms) - day_ms) <= 1);
    ...
    TIME_SHIELD_TEST_CHECK(std::fabs(pre_base_oa_ms - expected_pre_base_oa_ms) < 2e-12);
    TIME_SHIELD_TEST_CHECK(std::llabs(oadate_to_ts_ms(pre_base_oa_ms) - pre_base_ms) <= 1);

    const jd_t jd_epoch = ts_to_jd(static_cast<ts_t>(0));
    TIME_SHIELD_TEST_CHECK(std::fabs(jd_epoch - 2440587.5) < epsilon);
    ...
    TIME_SHIELD_TEST_CHECK(std::fabs(phase_epoch - 0.7520754628736458) < 1e-12);

    const double age_epoch = moon_age_days(static_cast<fts_t>(0));
    TIME_SHIELD_TEST_CHECK(std::fabs(age_epoch - 22.209231150442246) < 1e-9);
