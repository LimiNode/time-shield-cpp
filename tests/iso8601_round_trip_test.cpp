#include <time_shield/text/time_formatting.hpp>
#include <time_shield/text/time_parser.hpp>
#include <time_shield/conversions.hpp>
#include "test_assert.hpp"

/// \brief ISO8601 round-trip tests for various offsets and precisions.
int main() {
    using namespace time_shield;

    const ts_t base_ts = to_timestamp(2024, 3, 20, 12, 34, 56);

    std::string str_z = to_iso8601_utc(base_ts);
    ts_t parsed_z = ts(str_z);
    TIME_SHIELD_TEST_CHECK(parsed_z == base_ts);

    std::string str_pos = to_iso8601(base_ts, SEC_PER_HOUR);
    ts_t parsed_pos = ts(str_pos);
    TIME_SHIELD_TEST_CHECK(parsed_pos == base_ts);

    const tz_t offset_neg = -(5 * SEC_PER_HOUR + 30 * SEC_PER_MIN);
    std::string str_neg = to_iso8601(base_ts, offset_neg);
    ts_t parsed_neg = ts(str_neg);
    TIME_SHIELD_TEST_CHECK(parsed_neg == base_ts);

    const ts_ms_t base_ms = ts_ms(2024, 3, 20, 12, 34, 56, 789);
    std::string str_ms = to_iso8601_ms(base_ms);
    ts_ms_t parsed_ms = ts_ms(str_ms);
    TIME_SHIELD_TEST_CHECK(parsed_ms == base_ms);

    std::string str_ms_z = to_iso8601_utc_ms(base_ms);
    ts_ms_t parsed_ms_z = ts_ms(str_ms_z);
    TIME_SHIELD_TEST_CHECK(parsed_ms_z == base_ms);

    std::string str_ms_pos = to_iso8601_ms(base_ms, SEC_PER_HOUR);
    ts_ms_t parsed_ms_pos = ts_ms(str_ms_pos);
    TIME_SHIELD_TEST_CHECK(parsed_ms_pos == base_ms);

    std::string str_ms_neg = to_iso8601_ms(base_ms, offset_neg);
    ts_ms_t parsed_ms_neg = ts_ms(str_ms_neg);
    TIME_SHIELD_TEST_CHECK(parsed_ms_neg == base_ms);

    ts_ms_t parsed_fail = 0;
    bool is_ok = str_to_ts_ms("2024-03-20T12:34:56.789123Z", parsed_fail);
    TIME_SHIELD_TEST_CHECK(!is_ok);

    is_ok = str_to_ts_ms("2024-03-20T12:34:56.789123+01:00", parsed_fail);
    TIME_SHIELD_TEST_CHECK(!is_ok);

    is_ok = str_to_ts_ms("2024-03-20T12:34:56.789123-05:30", parsed_fail);
    TIME_SHIELD_TEST_CHECK(!is_ok);

    (void)base_ts;
    (void)str_z;
    (void)parsed_z;
    (void)str_pos;
    (void)parsed_pos;
    (void)offset_neg;
    (void)str_neg;
    (void)parsed_neg;
    (void)base_ms;
    (void)str_ms;
    (void)parsed_ms;
    (void)str_ms_z;
    (void)parsed_ms_z;
    (void)str_ms_pos;
    (void)parsed_ms_pos;
    (void)str_ms_neg;
    (void)parsed_ms_neg;
    (void)parsed_fail;
    (void)is_ok;

    return 0;
}
