...
int main() {
    ...
    TIME_SHIELD_TEST_CHECK(clock.use_ntp());
    TIME_SHIELD_TEST_CHECK(!clock.ntp_active());
    ...
    TIME_SHIELD_TEST_CHECK(clock.ntp_active());
    TIME_SHIELD_TEST_CHECK(NtpTimeService::instance().measure_count() >= 1);
    TIME_SHIELD_TEST_CHECK(NtpTimeService::instance().offset_us() >= 1000);
    TIME_SHIELD_TEST_CHECK(utc_us > realtime_before);
    TIME_SHIELD_TEST_CHECK(utc_us - realtime_before >= 500);
    TIME_SHIELD_TEST_CHECK(utc_us <= realtime_after + static_cast<ts_us_t>(NtpTimeService::instance().offset_us()) + 100000);
    ...
    TIME_SHIELD_TEST_CHECK(snapshot.utc_offset() == SEC_PER_HOUR);
