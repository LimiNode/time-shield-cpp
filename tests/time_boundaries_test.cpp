...
int main() {
    ...
    TIME_SHIELD_TEST_CHECK(dec_end + 1 == jan_start);
    TIME_SHIELD_TEST_CHECK(dec_start + SEC_PER_DAY == jan_start);
    TIME_SHIELD_TEST_CHECK(dec_end + SEC_PER_DAY == jan_end);

    ts_t start_day = ts("2020-01-01T00:00:00Z");
    ts_t end_day = ts("2020-01-01T23:59:59Z");
    TIME_SHIELD_TEST_CHECK(end_day - start_day == SEC_PER_DAY - 1);

    ts_ms_t frac_ms = ts_ms("2020-01-01T00:00:00.123Z");
    ts_ms_t start_ms = ts_ms("2020-01-01T00:00:00Z");
    TIME_SHIELD_TEST_CHECK(frac_ms - start_ms == 123);
