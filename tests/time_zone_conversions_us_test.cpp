...
int main() {
    ...
    TIME_SHIELD_TEST_CHECK(gmt_ct_standard == to_timestamp(2024, 1, 15, 18, 0, 0));

    ts_t et_standard = to_timestamp(2024, 1, 15, 12, 0, 0);
    ts_t gmt_et_standard = et_to_gmt(et_standard);
    TIME_SHIELD_TEST_CHECK(gmt_et_standard == to_timestamp(2024, 1, 15, 17, 0, 0));

    ts_t ct_summer = to_timestamp(2024, 7, 15, 12, 0, 0);
    ts_t gmt_ct_summer = ct_to_gmt(ct_summer);
    TIME_SHIELD_TEST_CHECK(gmt_ct_summer == to_timestamp(2024, 7, 15, 17, 0, 0));

    ts_t et_summer = to_timestamp(2024, 7, 15, 12, 0, 0);
    ts_t gmt_et_summer = et_to_gmt(et_summer);
    TIME_SHIELD_TEST_CHECK(gmt_et_summer == to_timestamp(2024, 7, 15, 16, 0, 0));

    ts_t ct_wrap_check = to_timestamp(2024, 6, 10, 9, 0, 0);
    TIME_SHIELD_TEST_CHECK(ct_to_gmt(ct_wrap_check) == et_to_gmt(ct_wrap_check + SEC_PER_HOUR));

    ts_t gmt_wrap_check = to_timestamp(2024, 6, 10, 15, 0, 0);
    TIME_SHIELD_TEST_CHECK(gmt_to_ct(gmt_wrap_check) == gmt_to_et(gmt_wrap_check) - SEC_PER_HOUR);
    ...
    TIME_SHIELD_TEST_CHECK(gmt_et_before_spring == to_timestamp(2024, int(MAR), spring_day_2024, 6, 59, 59));

    ts_t et_after_spring = to_timestamp(2024, int(MAR), spring_day_2024, 3, 0, 0);
    ts_t gmt_et_after_spring = et_to_gmt(et_after_spring);
    TIME_SHIELD_TEST_CHECK(gmt_et_after_spring == to_timestamp(2024, int(MAR), spring_day_2024, 7, 0, 0));
    ...
    TIME_SHIELD_TEST_CHECK(gmt_after_spring == to_timestamp(2024, int(MAR), spring_day_2024, 8, 0, 0));
    ...
    TIME_SHIELD_TEST_CHECK(gmt_et_repeat_hour == to_timestamp(2024, int(NOV), fall_day_2024, 5, 30, 0));

    ts_t ct_repeat_hour = to_timestamp(2024, int(NOV), fall_day_2024, 1, 30, 0);
    ts_t gmt_repeat_hour = ct_to_gmt(ct_repeat_hour);
    TIME_SHIELD_TEST_CHECK(gmt_repeat_hour == to_timestamp(2024, int(NOV), fall_day_2024, 7, 30, 0));

    ts_t ct_after_fall = to_timestamp(2024, int(NOV), fall_day_2024, 2, 0, 0);
    ts_t gmt_after_fall = ct_to_gmt(ct_after_fall);
    TIME_SHIELD_TEST_CHECK(gmt_after_fall == to_timestamp(2024, int(NOV), fall_day_2024, 8, 0, 0));
